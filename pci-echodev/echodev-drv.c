...
static ssize_t echo_write(struct file *file, const char __user *user_buffer, size_t count, loff_t *offs)
{
	...
	if(*offs >= pci_resource_len(echo->pdev, 1)) return 0;
...
static ssize_t echo_read(struct file *file, char __user *user_buffer, size_t count, loff_t *offs)
{
	char *buf;
	struct echodev *echo = (struct echodev *)file->private_data;
	int not_copied, to_copy = (count + *offs < 4096) ? count : pci_resource_len(echo->pdev, 1) - *offs;
	...
	not_copied = copy_from_user(user_buffer, buf, to_copy);
...
static int echo_probe(struct pci_dev *pdev, const struct pci_device_id * id)
{
	...
	echo->dev_nr = MKDEV(DEVNE, card_count++);
	...
	if(!echo->ptr_bar0)
	{
		printk("echo-drv - Error mapping BAR0\n");
		status = -ENODEV;
		goto fdev;
	}
	...
	status = devm_request_irq(&pdev->dev, irq_nr, echo_irq_handler, 0, "echodev-drv-irq", echo);
	if(status != 0)
	{
...
fdev :
	/* Removing ecgo from list is missing */
	cdev_del(&echo->cdev);
	return status;
}
...
static void echo_remove(struct pci_dev *pdev)
{
	...
	printk("echo-drv - Removing the Device whith Device Number %d : %d\n",
...
static int __init echo_init(void)
{
	...
	int status;
	dev_t dev_nr = MKDEV(DEVNE, 0);

	status = register_chrdev_region(dev_nr, MINORMASK + 1, DEVNENAME);
	if(status < 0)
	{	
		printk("echodev-drv - Error register Device Numbers\n");
		return status;
	}
