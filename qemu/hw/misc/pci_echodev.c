...
static uint64_t pciechodev_bar0_mmio_read(void *opaque, hwaddr addr, unsigned size)
{
    ...
    printf("PCIECHODEV: pciechodev_mmio_read() addr %lx size %x \n", addr, size);
...
static void pciechodev_bar0_mmio_write(void *opaque, hwaddr addr, uint64_t val,
                unsigned size)
{
    printf("PCIECHODEV: pciechodev_mmio_write() addr %lx size %x val %lx \n", addr, size, val);
    PciechodevState *pciechodev = opaque;
...
static void pci_pciechodev_realize(PCIDevice *pdev, Error **errp)
{
    ...
    memory_region_init_io(&pciechodev->mmio_bar1, OBJECT(pciechodev), 
                        &pciechodev_bar1_mmio_ops, pciechodev, "pciechodev-mmio", 4096);
    /* Registering all of above configuration */
...
static void pciechodev_class_init(ObjectClass *class, void *data)
{
    ...
    k->device_id = 0xBEEF; //our device id, 'abcd' hexadecimal
    k->revision = 0x10;
